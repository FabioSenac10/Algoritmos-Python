...
def cadastrar_veiculo():
    ...
    while True:
        ...
        if ano.isdigit() and len(ano) == 4:
            ...
            print("Ano inválido. Digite um ano com 4 dígitos.")
...
def consultar_os():
    opcao = input("Consultar por (1) CPF ou (2) Número da OS? ").strip()
    if opcao == '1':
        cpf = input("Digite o CPF: ").strip()
        with open('ordens_servico.txt', 'r') as f:
            for linha in f:
...
def editar_veiculo():
    placa = input("Placa do veículo a editar: ").strip().upper()
...
def excluir_cliente():
    ...
    print("Cliente e dados associados removidos.")

def excluir_veiculo():
    placa = input("Placa do veículo a excluir: ").strip().upper()
    if not buscar_veiculo_por_placa(placa):
        print("Veículo não encontrado.")
        return
    remover_linha('veiculos.txt', placa)
    remover_linhas_por_campo('ordens_servico.txt', 4, placa)
    print("Veículo e ordens associadas removidos.")
